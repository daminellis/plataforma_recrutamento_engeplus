...
export class GenerateMigration1730771693231 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`candidatura\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nome_completo\` varchar(50) NOT NULL, \`email\` varchar(50) NOT NULL, \`telefone\` text NOT NULL, \`descricao\` text NOT NULL, \`cv_data\` longblob NOT NULL, \`cv_type\` varchar(100) NOT NULL, \`data_candidatura\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`favorito\` tinyint NOT NULL DEFAULT 0, \`status\` enum ('Ativo', 'Inativo', 'Aprovado', 'Reprovado') NOT NULL DEFAULT 'Ativo', \`vagaId\` int NOT NULL, \`candidaturaTagIdId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
