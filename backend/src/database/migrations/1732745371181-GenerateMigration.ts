...
export class GenerateMigration1732745371181 implements MigrationInterface {
    name = 'GenerateMigration1732745371181'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`cargo\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nome\` varchar(50) NOT NULL, \`descricao\` varchar(100) NOT NULL, UNIQUE INDEX \`IDX_5511f6fba69ccf2c6147d98319\` (\`nome\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`usuario\` (\`id\` int NOT NULL AUTO_INCREMENT, \`username\` varchar(50) NOT NULL, \`nome_completo\` varchar(50) NOT NULL, \`email\` varchar(50) NOT NULL, \`senha_hash\` varchar(70) NOT NULL, \`tipo_usuario\` enum ('Administrador', 'Recursos Humanos', 'Líder') NOT NULL, \`cargoId\` int NULL, \`setorId\` int NULL, UNIQUE INDEX \`IDX_6ccff37176a6978449a99c82e1\` (\`username\`), UNIQUE INDEX \`IDX_2863682842e688ca198eb25c12\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`banco_talentos\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nome_completo\` varchar(50) NOT NULL, \`email\` varchar(50) NOT NULL, \`telefone\` varchar(11) NOT NULL, \`descricao\` text NOT NULL, \`cv_data\` longblob NOT NULL, \`cv_type\` varchar(50) NOT NULL, \`vaga_id\` int NOT NULL, \`titulo_vaga\` varchar(100) NOT NULL, \`descricao_vaga\` text NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`vaga_tags_tag\` ADD CONSTRAINT \`FK_a3ce448847f1a94723bfd869c6b\` FOREIGN KEY (\`tagId\`) REFERENCES \`tag\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
